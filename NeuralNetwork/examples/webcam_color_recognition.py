...
class WebcamColorApp(QtWidgets.QMainWindow):
    ...
    def setup_ui(self):
        self.setWindowTitle("Neural Network Webcam Color Recognition (Backprop)")
        ...
        controls_info_layout.addWidget(QtWidgets.QLabel("Sampled Color Preview:")); controls_info_layout.addWidget(self.color_preview)
        sampling_group = QtWidgets.QGroupBox("Color Sampling for Training")
        ...
        self.color_target_combo.setToolTip("Select the color category for the current sample.")
        ...
        self.sample_color_btn.setToolTip("Capture the current center color and assign it to the selected category for training.")
        ...
        self.train_network_btn.setToolTip("Train the network using all collected color samples.")
        ...
        main_layout.addWidget(self.webcam_label, 2); main_layout.addWidget(right_panel_widget, 1)
    ...
    def setup_network(self):
        ...
        self.network.add_neuron(input_names[0],value=0,position=(100,100),neuron_type="input",attributes={'color':(255,100,100)})
        self.network.add_neuron(input_names[1],value=0,position=(100,200),neuron_type="input",attributes={'color':(100,255,100)})
        self.network.add_neuron(input_names[2],value=0,position=(100,300),neuron_type="input",attributes={'color':(100,100,255)})
        for i,name in enumerate(hidden_names): self.network.add_neuron(name,value=50,position=(280,70+i*70),neuron_type="hidden")
        output_colors_map={"red_out":(220,50,50),"green_out":(50,200,50),"blue_out":(50,50,220),"yellow_out":(220,220,50),"other_out":(128,128,128)}
        for i,name in enumerate(self.output_neuron_names_for_sampling): self.network.add_neuron(name,value=50,position=(450,70+i*70),neuron_type="output",attributes={'color':output_colors_map[name]})
